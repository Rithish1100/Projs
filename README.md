# Python Projects

This repository contains a collection of Python mini-projects I built for learning and practice.  
Each project focuses on a different programming concept such as loops, functions, conditionals, and logic building.

---

## 📂 List of Projects

- [day-1.py](day-1.py) – Treasure Hunt Game  
- [day-2.py](day-2.py) – Password Generator
- [day-3.py](day-3.py) – Rock, Paper, Scissors Game
- [day-5.py](day-5.py) –  Hangman Game 
- [day-6.py](day-6.py) –  Caesar Cipher – Encode and Decode 
- [day-7.py](day-7.py) – Hangman 
- [day-8.py](day-8.py) –Bidding Game 
- [day-9.py](day-9.py) - calculator
- [day-10.py](day-10.py) - Black Jack Game
- [day-12.py](day-12.py) - Number Guessing

---

## 🚀 How to Run These Projects

1. Clone the repository:
   ```bash
   git clone https://github.com/Rithish1100/Projs.git
